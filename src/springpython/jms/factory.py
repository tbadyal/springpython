...
class WebSphereMQConnectionFactory(DisposableObject):
    ...
    def __init__(self, queue_manager=None, channel=None, host=None, listener_port=None,
        ...
        self.queue_manager = queue_manager.encode('utf8')
...
class MQRFH2JMS(object):
    ...
    def add_jms(self, message, queue_name, now):
        ...
        dlv.text = str(message.jms_delivery_mode)

        if message.jms_expiration:
            exp = etree.Element("Exp")
            exp.text = str(now + message.jms_expiration)
            self.logger.log(TRACE1, "jms.Exp [%r]" % exp.text)
            jms.append(exp)

        if message.jms_priority:
            pri = etree.Element("Pri")
            pri.text = str(message.jms_priority)
            self.logger.log(TRACE1, "jms.Pri [%r]" % pri.text)
            jms.append(pri)

        if message.jms_correlation_id:
            cid = etree.Element("Cid")
            cid.text = str(message.jms_correlation_id)
    ...
    def add_usr(self, message):
        ...
        if user_attrs:
            ...
            for user_attr in user_attrs:
                ...
                user_attr = etree.Element(str(user_attr))
                user_attr.text = str(user_attr_value)
